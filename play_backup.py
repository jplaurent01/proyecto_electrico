...
class Application(Toplevel):
    ...
    def goto_next_question(self):
        ...
        if len(question["text"]) >= 20 and len(question["text"]) <=45:
            self.question_label.place(relx = 0.32, rely = 0.59)
        elif len(question["text"]) < 20:
            ...
            self.question_label.place(relx = 0.19, rely = 0.59)
        elif len(question["text"]) > 72 and len(question["text"])<= 81:
            self.question_label.configure(font=("Arial", 16))
            self.question_label.place(relx = 0.18, rely = 0.59)
        elif len(question["text"]) > 81 and len(question["text"])<= 90:
            self.question_label.configure(font=("Arial", 13))
            self.question_label.place(relx = 0.17, rely = 0.59)
        else:
            self.question_label.configure(font=("Arial", 19))
            self.question_label.place(relx = 0.16, rely = 0.59)
    ...
    def plot(self, question):
        ...
        #self.label_bg_prb.place(relx = 0.4, rely = 0.1)

        #ax = plt.axes()

        #ax.spines['bottom'].set_color('#ECEEF1')
        #ax.spines['top'].set_color('#ECEEF1')
        #ax.spines['right'].set_color('#ECEEF1')
        #ax.spines['left'].set_color('#ECEEF1')
        #self.plot1.yaxis.label.set_color('#ECEEF1')
        #self.plot1.xaxis.label.set_color('#ECEEF1')
        # creating the Tkinter canvas
        # containing the Matplotlib figure
        #self.canvas = FigureCanvasTkAgg(self.fig,master = self.frm)
        #self.canvas.draw()
...
def main(root, count):
    #qst = List = open(r"quien quiere ser millonario\Dependencies\preguntas.txt").readlines()
    ...
    print("Cantidad de lineas: " + str(count_line))
    if count_line >= 15:
